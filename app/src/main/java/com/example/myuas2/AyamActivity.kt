...
class AyamActivity : AppCompatActivity() {
    ...
    private lateinit var AyamAdapter: Adapter
...
//    }
    ...
    private fun setupList(){
        ...
        AyamAdapter = Adapter(arrayListOf())
        lisNote.adapter = AyamAdapter
    }
        ...
        api.data().enqueue(object : Callback<Model> {
            override fun onResponse(call: Call<Model>, response: Response<Model>) {
                if (response.isSuccessful){
                    val listData = response.body()!!.ayam
                    AyamAdapter.setData( listData )
//                    listData.forEach {
